...
from db.dal.dals import TaskDAL, TaskStatus
from dependencies.db import get_db_dal_by_type
from schemas.schemas import TaskScheme, TaskCreateScheme, TaskUpdateScheme, CommentScheme, EmployeeChangeScheme, \
    StatusChangeScheme, EventScheme, CommentCreateScheme, EmployeeChangeCreateScheme, StatusChangeCreateScheme
...
async def filter_tasks(
    ...
    tasks = await task_dal.filter_tasks(start_time=start_time, end_time=end_time, employee_id=employee_id, status=status)
    return [TaskScheme.from_orm(task) for task in tasks]
...
async def get_all_related_objects_for_task(
...
) -> typing.Sequence[typing.Union[CommentScheme, EmployeeChangeScheme, StatusChangeScheme]]:
    task = await task_dal.get_by_id_with_exception(task_id)
    ...
    employee_change_schemes = [EmployeeChangeScheme.from_orm(ec) for ec in employee_changes]
    ...
    related_object_schemes = comment_schemes + employee_change_schemes + status_change_schemes
...
async def create_event_comment(
    ...
    comment = await task_dal.create_event_comment_with_event(task_id, employee_id, comment_data)
    return CommentScheme.from_orm(comment)


@router.post("/{task_id}/{employee_id}/events/employee-changes", response_model=EmployeeChangeScheme)
async def create_event_employee_change(
    ...
    employee_change = await task_dal.create_event_employee_change_with_event(task_id, employee_id, employee_change_data)
    return EmployeeChangeScheme.from_orm(employee_change)


@router.post("/{task_id}/{employee_id}/events/status-changes", response_model=StatusChangeScheme)
async def create_event_status_change(
    ...
    status_change = await task_dal.create_event_status_change_with_event(task_id, employee_id, status_change_data)
